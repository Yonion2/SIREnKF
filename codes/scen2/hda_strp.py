from EnKF_both_strap import EnKFBoth
from EnKF_delta import EnKFBeta
from EnKF_delta_gamma import EnKFGamma
from utils import save_obj, load_obj, hx2, hx, map, reverse_map
...
def run(gt_param, beta, gamma, task, Q_x, Q_param, P_x, P_param, R_x, N, windows, rounds, measurement_mode='infection'):
    ...
    post_states = list()
    prior_states = list()
    save_path = 'beta{}_gamma{}_mea{}_Qx{}_Qp{}_Px_{}_Pp{}_Rx{}_N{}_L{}3'.format(
...
if __name__ == '__main__':
    ...
    run(gt_param = gt_param, beta=0.01, gamma=0.01, task='all', Q_x = 1e-4, Q_param = 1e-4, P_x = 5e-4, P_param = 1e-2, R_x= 5e-3, N = 75, windows = 10, rounds = 3000, measurement_mode='both')