...
# def reverse_map(y):
#     return math.atan(y/300) / math.pi + 1/2
...
class EnKFBoth(object):
    ...
    def initialize(self, x, P):
        ...
        sig = deepcopy(self.sigmas)
        self.collect.append(sig)
    ...
    def update(self, z, R=None):
        ...
        self.collect.append(deepcopy(self.sigmas))
    ...
    def predict(self, Q=None, windows=5):
        ...
        for i, s in enumerate(self.sigmas):
            if self.task == 'all':
                self.sirmodels[i].params['model']['beta'] = deepcopy(reverse_new(self.sirmodels[i].params['model']['beta'], reverse_map(self.sigmas[i][-2]), reverse_map(self.collect[-2][i][-2])))
                self.sirmodels[i].params['model']['gamma'] = deepcopy(reverse_new(self.sirmodels[i].params['model']['gamma'], reverse_map(self.sigmas[i][-1]), reverse_map(self.collect[-2][i][-1])))
