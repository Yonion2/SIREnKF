...
class EnsembleKalmanFilterBeta(object):
    """
    """
    ...
    def predict(self, Q=None, windows=1):
        ...
        for i, s in enumerate(self.sigmas):
            if self.task == 'all':
                # self.sirmodels[i].parameters['model']['beta'] = reverse_map(self.sigmas[i][-2])
                self.sirmodels[i].params['model']['beta'] = reverse_map(self.sigmas[i][-2])  # 修正模型种的超参数值
                # self.sirmodels[i].parameters['model']['gamma'] = reverse_map(self.sigmas[i][-1])
                self.sirmodels[i].params['model']['gamma'] = reverse_map(self.sigmas[i][-1])
            elif self.task == 'beta':
                # print('1: ', self.sirmodels[i].parameters['model']['beta'])
                # print('2: ', self.sirmodels[i].params['model']['beta'])
                # self.sirmodels[i].parameters['model']['beta'] = reverse_map(self.sigmas[i][-1])
                self.sirmodels[i].params['model']['beta'] = reverse_map(self.sigmas[i][-1])
            else:
                assert self.task == 'gamma'
                # self.sirmodels[i].parameters['model']['gamma'] = reverse_map(self.sigmas[i][-1])
                self.sirmodels[i].params['model']['gamma'] = reverse_map(self.sigmas[i][-1])
